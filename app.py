...
class Entry(flask_db.Model):
    ...
    def html_content(self):
        ...
        oembed_content = parse_html(
            ...
            oembed_providers,
            urlize_all=True,
            maxwidth=flask_app.config['SITE_WIDTH'])
...
def login():
    ...
    if request.method == 'POST' and request.form.get('password'):
        ...
        if password == flask_app.config['ADMIN_PASSWORD']:
...
@flask_app.route('/<slug>/edit/', methods=['GET', 'POST'])
@login_required
def edit(slug):
    entry = get_object_or_404(Entry, Entry.slug == slug)
    return _create_or_edit(entry, 'edit.html')

@flask_app.template_filter('clean_querystring')
